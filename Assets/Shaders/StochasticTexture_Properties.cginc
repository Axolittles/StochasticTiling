#ifndef STOCHASTICTILINGPROPERTIES_INCLUDED
#define STOCHASTICTILINGPROPERTIES_INCLUDED
...
void StochasticTilingProperties_float(
    ...
    out float Smoothness)
{
    ...
    float2 l1 = local; if (m1) l1.x = 1.0 - l1.x;
    float2 l2 = local; if (m2) l2.x = 1.0 - l2.x;
    float2 l3 = local; if (m3) l3.x = 1.0 - l3.x;
    ...
    // --- Height-aware reweighting (softmax over heights), mixed with barycentric ---
    float h1 = p1.x;
    float h2 = p2.x;
    float h3 = p3.x;

    // Reference everything to the local max
    float hMax = max(h1, max(h2, h3));    
    float g1 = exp((h1 - hMax) * HeightContrast);
    float g2 = exp((h2 - hMax) * HeightContrast);
    float g3 = exp((h3 - hMax) * HeightContrast);
